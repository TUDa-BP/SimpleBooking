...
import PlaceBookingSlotButton from '@/components/PlaceBookingSlotButton.vue';

describe('PlaceBookingSlotButton.vue', () => {
    const slot = ref({
        isBookableForYou: true,
        start_time: new Date('2023-10-10T10:00:00'),
        end_time: new Date('2023-10-10T11:00:00'),
        booking_selected_booking_type: 'reserved',
        booking: {
    ...
    it('renders correctly when slot is bookable', () => {
        const wrapper = mount(PlaceBookingSlotButton, {
            props: {
                slot: slot.value,
    ...
    it('applies correct class based on booking type', () => {
        ...
        expect(wrapper.find('button').classes()).toContain('booking-type-reserved');
    });
});